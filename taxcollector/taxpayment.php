...
$result=$mysqli->query("select *   from taxpayment where STATUS='collected'") or die(mysqli_error($mysqli));
$query="SELECT sum(AMOUNT) AS total from taxpayment" ;
        ...
        <div class="panel-body">
           <form id="signup_form" method="POST" class="login100-form">
                                     ...
                                     <div class="form-group">
                                        <label>Total cash From Tax payment  KSH</label>
                                        <?php
//host name
$host = "localhost";
//user name
$username = "id14504012_root";
//database password
$password = "V_vtyJ9Pffk9Hi|Y";
//database name
...
$mysqli = new mysqli($host,$username,$password,$database) or die($mysqli->error);
                           ...
                           if(isset($_SESSION['uname']))
                              {
                               $result=$mysqli->query("select *  from taxpayment where STATUS='collected'") or die(mysqli_error($mysqli));
                               $query="SELECT sum(AMOUNT) AS total from taxpayment where STATUS='collected'" ;
                               $result2=$mysqli->query($query) or die(mysqli_error($mysqli));
                               $row1=$result2->fetch_assoc();
                              
                                        if (mysqli_num_rows($result) > 0) {      
                                   $result=$mysqli->query("select * from taxpayment where STATUS='collected' ")or die($mysqli->error);
         ...
         <table id="example2" class="table table-responsive table-hover">
                  ...
                  while($row=$result->fetch_assoc())
                  {
                    ...
                    <td>".$row['AMOUNT']."</td>
                    <td>".$row['STATUS']."</td>
                    <td> <a href='taxpayment.php?apid=$row[PAYMENT_ID]' class='btn btn-success'>collected</a>
                    <a href='taxpayment.php?rpid=$row[PAYMENT_ID]' class='btn btn-danger'>Reject</a>
...
<td><a href='paymentreport2.php' class='btn btn-danger'>Export to PDF</a></td>
    ...
    if(isset($_GET['apid']))
  {
    ...
    $result = $mysqli->query("UPDATE taxpayment SET STATUS= 'collected' WHERE PAYMENT_ID = $payment_id") or die($mysqli->error);
